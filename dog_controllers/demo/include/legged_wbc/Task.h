...
class Task {
 public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW  // 确保Eigen对象内存对齐
  ...
  static matrix_t concatenateMatrices(matrix_t m1, matrix_t m2) {
    if (m1.cols() <= 0) {  // 如果m1为空，返回m2
      return m2;
    } else if (m2.cols() <= 0) {  // 如果m2为空，返回m1
      return m1;
    }
  ...
  static vector_t concatenateVectors(const vector_t& v1, const vector_t& v2) {
    if (v1.cols() <= 0) {  // 如果v1为空，返回v2
      return v2;
    } else if (v2.cols() <= 0) {  // 如果v2为空，返回v1
      return v1;
    }
